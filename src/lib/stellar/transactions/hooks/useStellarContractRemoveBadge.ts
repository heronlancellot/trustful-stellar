...
export const useStellarContractRemoveBadge = ({ contractId, rpcUrl, networkType = "TESTNET" }: UseStellarContractRemoveBadgeProps) => {

    const executeContractFunction = async (functionName: string, badgeName: string, issuer: string) => {
    ...
    return {
        ...
        removeBadge: (badgeName: string, issuer: string) => executeContractFunction('remove_badge', badgeName, issuer),
    };
}; 